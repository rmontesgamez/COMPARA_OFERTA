...
HAVING (((PIEZAS.REF_C) Like '%""" + self.e_pedido + "%') AND ((PEDIDOS.COMPLETADO)='N')) ORDER BY PEDIDOS.PEDIDO;"
           ...
           SELECT Max((PIEZAS.REF_N))  FROM PIEZAS;"""
            ...
            sql_query=("""
            SELECT OFERTAS.OFERTA, PZOFERTA.REF, PIEZAS.TIPOM, PIEZAS.REF_N, PIEZAS.C_VR, PZOFERTA.QPZ, PIEZAS.TRATMTO, PZOFERTA.VPU, PZOFERTA.VPZ, PZOFERTA.PROPMAT, PZOFERTA.PR_TRANSPORTE, PIEZAS.VGAS, PIEZAS.VCORTE, PZOFERTA.VTRATMTO
FROM CLIENTES INNER JOIN (PIEZAS INNER JOIN (OFERTAS INNER JOIN PZOFERTA ON OFERTAS.OFERTA = PZOFERTA.OFERTA) ON (PIEZAS.C_VR = PZOFERTA.C_VR) AND (PIEZAS.REF_C = PZOFERTA.REF)) ON (CLIENTES.CODIGO = PIEZAS.CODCLIENTE) AND (CLIENTES.CODIGO = OFERTAS.CODCLIENTE)
