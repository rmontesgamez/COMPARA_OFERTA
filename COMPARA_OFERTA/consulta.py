...
HAVING (((PIEZAS.REF_C) Like '%""" + self.e_pedido + "%') AND ((PEDIDOS.COMPLETADO)='N')) ORDER BY PEDIDOS.PEDIDO;"
           ...
           SELECT Max((PIEZAS.REF_N))  FROM PIEZAS;"""
            ...
            sql_query=("""
            SELECT OFERTAS.OFERTA, PZOFERTA.REF, PIEZAS.REF_N, PIEZAS.C_VR, PZOFERTA.VPU, PZOFERTA.VPZ, PZOFERTA.QPZ, PZOFERTA.PROPMAT, PZOFERTA.PR_TRANSPORTE, PIEZAS.VCORTE, PZOFERTA.VTRATMTO
FROM CLIENTES INNER JOIN (PIEZAS INNER JOIN (OFERTAS INNER JOIN PZOFERTA ON OFERTAS.OFERTA = PZOFERTA.OFERTA) ON (PIEZAS.REF_C = PZOFERTA.REF) AND (PIEZAS.C_VR = PZOFERTA.C_VR)) ON (CLIENTES.CODIGO = OFERTAS.CODCLIENTE) AND (CLIENTES.CODIGO = PIEZAS.CODCLIENTE)
